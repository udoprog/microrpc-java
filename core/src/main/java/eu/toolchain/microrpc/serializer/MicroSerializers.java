...
public class MicroSerializers {
    ...
    private final Serializer<MicroRequestHeartbeat> requestKeepAlive;
    ...
    public MicroSerializers(final SerializerFramework s) {
        ...
        this.requestKeepAlive = new Serializer<MicroRequestHeartbeat>() {
            private final Serializer<UUID> uuid = s.uuid();

            @Override
            public void serialize(SerialWriter buffer, MicroRequestHeartbeat value) throws IOException {
                uuid.serialize(buffer, value.getRequestId());
            }

            @Override
            public MicroRequestHeartbeat deserialize(SerialReader buffer) throws IOException {
                final UUID requestId = uuid.deserialize(buffer);
                return new MicroRequestHeartbeat(requestId);
            }
        };
        ...
        types.add(s.type(0x02, MicroRequestHeartbeat.class, requestKeepAlive()));
    ...
    public Serializer<MicroRequestHeartbeat> requestKeepAlive() {
