...
import eu.toolchain.microrpc.netty.NettyMicroNetwork;
...
public class ClientAndServerExample {
    public static MicroServer buildServer(final SerializerFramework s, final AsyncFramework async) {
        ...
        final MicroNetwork network = NettyMicroNetwork.builder().async(async).router(router).serializer(s).build();
        return MicroServer.builder().async(async).network(network).build();
    }
